...
## Efficient intrusion detection with Route Ranking AODV (RRAODV)


* 
	AODV figures a route to the destination whenever necessary, there may be a malicious node present in the route affecting the functionality and efficiency of the route and AODV. Whenever the source node intended to initiate a transfer of packets, the Blackhole node fakes an RREP to the source node and receives and drops the packet instead of forwarding it. So that more packets are dropped when transmission occurs between two nodes. 
	In order to resolve the excess amount of packet drop and lack in AODV security, we propose a mechanism that enhances the existing AODV routing protocol on Mobile Ad-hoc Network.
	In this paper, we propose two mechanisms that modify the existing AODV protocol. (i) Test packets are used to check the initial transmission status and  (ii) An additional field called Success Rate is added to the AODV routing protocol which is calculated from the test packets

* Success Rate(SR) =  Number of Packets sent /
						Number of Packets Received

*  ADVANTAGES

    • In the proposed system we can identify and avoid the malicious nodes.

    • Packet Drop is reduced in higher amount.

    • Performance of the network will be increased.
    
* We used C++ and NS3 for this project 
...
![alt text](https://github.com/MuruganVengkadaeswara/RRAODV/blob/master/CD/Screenshot%20from%202019-04-03%2016-41-22.png?raw=true)
